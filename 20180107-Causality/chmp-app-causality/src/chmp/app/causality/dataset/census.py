...
columns = [
    ...
    ("education", "category"),
    ("wage per hour", "float"),
    ("enrolled in edu inst last wk", "category"),
    ...
    ("divdends from stocks", "float"),
    ("tax filer status", "category"),
    ("region of previous residence", "category"),
    ("state of previous residence", "category"),
    ("household and family stat", "category"),
    ("household summary in household", "category"),
    ...
    ("fill inc questionnaire for veterans admin", "category"),
...
def create(data_path):
    ...
    if not os.path.exists(data_fname) or not os.path.exists(test_fname):
        raise RuntimeError(
            "Could not find input data. Download census-income.data.gz and census-income.test.gz from "
            + source_url
        )

    _logger.info("load census data")
    train_data = (
        pd.read_csv(data_fname, sep=",", header=None).pipe(rename).assign(train=1)
    )
    test_data = (
        pd.read_csv(test_fname, sep=",", header=None).pipe(rename).assign(train=0)
    )
...
def generate_census_data(train_data, test_data):
    ...
    df["action_p"] = (df["target"] == 1) * df["action_p"] + (df["target"] == 0) * (
        1 - df["action_p"]
    )
    ...
    df["outcome_p"] = (1 - df["action"]) * df["outcome_0_p_det"] + df["action"] * df[
        "outcome_1_p_det"
    ]
...
def assemble_census_policy_pipeline(C=5e-4):
    est = sklearn.pipeline.Pipeline(
        [
            ...
            (
                "predict",
                sklearn.linear_model.LogisticRegression(C=C, random_state=15506),
            ),
        ]
    )
