...
def test_base_annotator__example_session():
    ...
    assert annotator.annotations[-1] == {'item': 'a', 'index': 0, 'reason': 'order', 'label': 'foo'}
    ...
    assert annotator.annotations[-1] == {'item': 'a', 'index': 0, 'reason': 'repeat', 'label': 'bar'}
    assert annotator.current_item == ('order', 1, 'b')

    annotator.annotate_current('baz')
    assert len(annotator.annotations) == 3
    assert annotator.annotations[-1] == {'item': 'b', 'index': 1, 'reason': 'order', 'label': 'baz'}
    assert annotator.current_item == ('order', 2, 'c')

    annotator.annotate_current('foo')
    assert len(annotator.annotations) == 4
    assert annotator.annotations[-1] == {'item': 'c', 'index': 2, 'reason': 'order', 'label': 'foo'}
    assert annotator.current_item is None
    ...
    assert annotator.annotations[-1] == {'item': 'b', 'index': 1, 'reason': 'repeat', 'label': 'fxx'}
    assert annotator.current_item is None
