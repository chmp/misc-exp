...
def evaluate(
        est, df,
        reward_column='outcome', action_column='action', value_columns=None, propensity_column=None,
        label=None,
):
    ...
    return _compute_metrics({
        ...
        'observed_action': np.asarray(df[action_column], dtype=np.int) if action_column is not None else None,
        'reward': df[reward_column] if reward_column is not None else None,
        'true_value': np.vstack([df[col] for col in value_columns]).T if value_columns is not None else None,
        'propensity': df[propensity_column] if propensity_column is not None else None,
    })
...
def compute_doubly_robust_reward(action, reward, propensity, observed_action, value, action_values, policy_p):
